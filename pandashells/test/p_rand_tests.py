...
from pandashells.bin.p_rand import fill_default_mu, get_samples, main

class FillDefaultMuTests(TestCase):
    def test_normal_mu_default(self):
        args = MagicMock(type=['normal'], mu=None)
        args = fill_default_mu(args)
        self.assertEqual(args.mu, [0.])

    def test_normal_defined_default(self):
    ...
    def test_other_default(self):
        args = MagicMock(type=['any_other_value'], mu=None)
        args = fill_default_mu(args)
        self.assertEqual(args.mu, None)
